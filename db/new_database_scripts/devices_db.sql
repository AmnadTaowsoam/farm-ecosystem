...
-- Trigger อัปเดต updated_at อัตโนมัติสำหรับ device_groups
...
-- 2.51) หมวดหมู่อุปกรณ์ (device_types)
...
CREATE INDEX IF NOT EXISTS idx_device_logs_device_id
  ON devices.device_logs(device_id);
...
CREATE INDEX IF NOT EXISTS idx_device_status_history_device_id
  ON devices.device_status_history(device_id);
