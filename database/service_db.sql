...
CREATE INDEX idx_dashboard_cache_farm_metric_date ON dashboard.dashboard_cache(farm_id, metric_name, metric_date);
...
CREATE INDEX idx_user_dashboard_config_user_id ON dashboard.user_dashboard_config(user_id);
...
CREATE TABLE monitoring.alerts (
    ...
    farm_id INTEGER REFERENCES core.farms(farm_id),
...
CREATE INDEX idx_alerts_farm_status ON monitoring.alerts(farm_id, status);
...
CREATE TABLE analytics.feature_store (
    ...
    animal_id INTEGER REFERENCES core.animals(animal_id),
...
CREATE INDEX idx_feature_store_animal_feature_date ON analytics.feature_store(animal_id, feature_name, feature_date);
...
CREATE INDEX idx_model_results_animal_model_date ON analytics.model_results(animal_id, model_name, result_date);
